...
def _bbox_from_docling(prov: Any) -> Optional[Bbox]:
    """Build Bbox from Docling provenance if available."""
    if prov is None:
        return None
...
def _page_from_docling(prov: Any) -> int:
    """Get 0-based page index from Docling item."""
    if prov is None:
        return 0
...
class DoclingDocumentAdapter:
    """Maps Docling document to our normalized schema with content_hash and reading_order."""
    ...
    def __init__(self, docling_document: Any, page_count: int, doc_id: str, source_path: Optional[Path] = None):
        ...
        self._page_index_to_page: dict[int, ExtractedPage] = {}
        for i in range(page_count):
            self._page_index_to_page[i] = ExtractedPage(page_index=i, text_blocks=[], tables=[], figures=[], reading_order=[])

    def _ensure_pages(self) -> None:
        """Populate pages from Docling document texts, tables, pictures."""
        doc = self.doc
        elem_id = 0
        reading_order_global: List[str] = []
        ...
        if hasattr(doc, "texts") and doc.texts:
            for t in doc.texts:
                ...
                eid = f"e{elem_id}"
                elem_id += 1
                reading_order_global.append(eid)
            ...
            for t in doc.tables:
                ...
                if hasattr(t, "export_to_dataframe"):
                    try:
                        ...
                        pass
                if not data and hasattr(t, "data"):
                    data = getattr(t, "data", [])
                if not data and hasattr(t, "cells"):
                    # Build grid from cells if needed
                    data = []
                bbox = _bbox_from_docling(prov)
        ...
        # If no structured items, try export_to_dict and parse
        if elem_id == 0 and hasattr(doc, "export_to_dict"):
            try:
                d = doc.export_to_dict()
                self._parse_export_dict(d, elem_id)
            except Exception:
                pass

    def _parse_export_dict(self, d: dict, start_id: int) -> None:
        """Fallback: parse export_to_dict() structure to fill pages."""
...
class LayoutExtractor:
    ...
    def extract(self, pdf_path: Path, profile: DocumentProfile) -> tuple[ExtractedDocument, float]:
        ...
        # Sanity check: table_heavy profile but 0 tables -> low confidence
        if profile.layout_complexity == "table_heavy" and total_tables == 0:
            confidence = 0.0
        else:
            confidence = 1.0
